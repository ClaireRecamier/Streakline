...
proc orbit (pos, vel, pot, integrator, N, dt, pos_lead, pos_trail, vel_lead, vel_trail) {
    ...
    for i in 1..N {//make N full steps in pos and vel forwards
      ...
      if i == 1 {
        ...
        myWritingChannel.write(pos[0][0],",",pos[0][1],",",vel[0][0],",",vel[0][1],",",pos_lead[k][0],",",pos_lead[k][1],",",vel_lead[k][0],",",vel_lead[k][1]);
        myWritingChannel.write(",",pos_trail[k][0],",",pos_trail[k][1],",",vel_trail[k][0],",",vel_trail[k][1],"\n");
      ...
      for j in 1..k {
        ...
        myWritingChannel.write(pos[i][0],",",pos[i][1],",",vel[i][0],",",vel[i][1],",",pos_lead[j][0],",",pos_lead[j][1],",",vel_lead[j][0],",",vel_lead[j][1]);
        myWritingChannel.write(",",pos_trail[j][0],",",pos_trail[j][1],",",vel_trail[j][0],",",vel_trail[j][1],"\n");
      }
    }
...
//  writeln("omega of cluster: ",omega);
  ...
  writeln("Rj: ",Rj);
  pos_lead = pos_cl - (0.25 * (pos_cl/r)); //multiplied by unit vector
  ...
  vel_lead = vel_cl * (mag_vcl - 0.25* om)/(mag_vcl);
  vel_trail = vel_cl * (mag_vcl + 0.25* om)/mag_vcl;
